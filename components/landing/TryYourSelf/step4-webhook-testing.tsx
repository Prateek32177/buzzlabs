...
import { ArrowLeft, Mail, MessageSquare, Play } from "lucide-react"
...
export function Step4WebhookTesting({
  ...
  const [showNotification, setShowNotification] = useState<"email" | "slack" | null>(null)
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="space-y-6">
      <div className="text-center">
        <h3 className="text-2xl font-bold text-white mb-2">Step 4: Test Your Webhook</h3>
        <p className="text-violet-200 mb-6">
          Trigger events and see your webhook in action with real-time notifications.
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-gray-800/50 backdrop-blur-sm rounded-lg border border-violet-800/30 p-6">
          <h4 className="text-lg font-medium text-white mb-4">Trigger Events</h4>

          {selectedApp && (
            <div className="space-y-3">
              <div className="bg-gray-900/50 p-3 rounded-md">
                <span className="text-violet-300 text-sm">Selected Application</span>
                <div className="text-white mt-1 font-medium capitalize">{selectedApp}</div>
              </div>

              <div className="bg-gray-900/50 p-3 rounded-md">
                <span className="text-violet-300 text-sm">Selected Webhook</span>
                <div className="text-white mt-1 font-medium">{selectedWebhook?.name}</div>
              </div>
                      ...
                      <div className="bg-gray-900 p-4 rounded-b-lg border border-gray-700 border-t-0">
                        <div className="space-y-3">
                          <div className="flex justify-between items-center">
                            ...
                            <span className="text-violet-400 font-medium capitalize">{currentEvent}</span>
                          </div>
                          <div className="flex justify-between items-center">
      ...
      <AnimatePresence>
        {showNotification === "email" && (
          <motion.div
            ...
            className="fixed top-0 right-0 transform -translate-x-1/2 z-50 w-80"
          >
            <div className="bg-white rounded-lg shadow-xl overflow-hidden">
              <div className="bg-red-500 px-4 py-2 flex items-center">
                <Mail className="text-white h-5 w-5 mr-2" />
                <span className="text-white font-medium">Gmail</span>
              </div>
              <div className="p-4">
                <h5 className="font-medium text-gray-900">Webhook Notification</h5>
                <p className="text-gray-700 text-sm mt-1">
                  {selectedApp} {currentEvent?.replace(/_/g, " ")} event triggered
                </p>
        ...
        {showNotification === "slack" && (
          ...
          >
            <div className="bg-white rounded-lg shadow-xl overflow-hidden">
              ...
              <div className="p-4">
                <h5 className="font-medium text-gray-900">Webhook Bot</h5>
                <p className="text-gray-700 text-sm mt-1">
      ...
      <div className="flex justify-between mt-6">
        ...
        <Button className="bg-violet-600 hover:bg-violet-700 text-white" onClick={() => window.location.reload()}>
          Start Over
        </Button>
      </div>
    </motion.div>
