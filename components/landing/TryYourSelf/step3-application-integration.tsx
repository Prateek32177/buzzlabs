...
const applicationEvents = {
  ...
  stripe: ["payment_succeeded", "payment_failed", "subscription_created", "subscription_updated", "refund_processed"],
}
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="space-y-6">
      <div className="text-center">
        <h3 className="text-2xl font-bold text-white mb-2">Step 3: Integrate with Applications</h3>
        <p className="text-violet-200 mb-6">
          Connect your webhook to applications and select the events you want to trigger.
        </p>
      </div>
      ...
      <div className="space-y-6">
        ...
        <div className="bg-gray-800/50 backdrop-blur-sm rounded-lg border border-violet-800/30 p-6">
          <h4 className="text-lg font-medium text-white mb-4">1. Select an Application</h4>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            {["supabase", "github", "stripe"].map((app) => (
              ...
              >
                ...
                {selectedApp === app && <Check className="h-5 w-5 text-violet-400" />}
              </motion.div>
            ))}
          </div>
        </div>
        ...
        {/* Webhook Selection */}
        <div className="bg-gray-800/50 backdrop-blur-sm rounded-lg border border-violet-800/30 p-6">
          <h4 className="text-lg font-medium text-white mb-4">2. Select a Webhook</h4>
          <Select value={selectedWebhook?.id} onValueChange={handleSelectWebhook} disabled={!selectedApp}>
          ...
          {selectedWebhook && (
            <div className="mt-4 space-y-3">
              <div className="bg-gray-900/50 p-3 rounded-md">
                ...
                <div className="text-white mt-1 font-mono text-sm truncate">{selectedWebhook.url}</div>
              </div>
              <div className="bg-gray-900/50 p-3 rounded-md">
                <Label className="text-violet-300 text-sm">Secret Key</Label>
                <div className="text-white mt-1 font-mono text-sm">••••••••••••••••••••••</div>
              </div>
            </div>
          )}
        </div>
        ...
        {selectedApp && (
          <div className="bg-gray-800/50 backdrop-blur-sm rounded-lg border border-violet-800/30 p-6">
            <h4 className="text-lg font-medium text-white mb-4">3. Select Events</h4>
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3">
              {applicationEvents[selectedApp as keyof typeof applicationEvents].map((event) => (
                <div key={event} className="flex items-center space-x-2">
                  ...
                  />
                  <label htmlFor={event} className="text-white cursor-pointer capitalize">
                    {event.replace(/_/g, " ")}
                  </label>
                </div>
              ))}
        ...
        {error && <div className="bg-red-900/30 border border-red-800 text-white p-3 rounded-md">{error}</div>}
      </div>
      ...
      <div className="flex justify-between mt-6">
        ...
        <Button
          ...
          disabled={!selectedApp || !selectedWebhook || selectedEvents.length === 0}
        >
