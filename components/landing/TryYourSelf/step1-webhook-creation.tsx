...
export function Step1WebhookCreation({ webhooks, setWebhooks, goToNextStep }: Step1Props) {
  const [webhookName, setWebhookName] = useState("")
  const [error, setError] = useState<string | null>(null)

  const generateWebhookUrl = () => {
    return `https://api.yourwebhookservice.com/webhook/${Math.random().toString(36).substring(2, 15)}`
  }
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="space-y-6">
      <div className="text-center">
        <h3 className="text-2xl font-bold text-white mb-2">Step 1: Create Your Webhook</h3>
        <p className="text-violet-200 mb-6">Start by creating a webhook. You can create up to 5 webhooks.</p>
      </div>
      ...
      <div className="bg-gray-800/50 backdrop-blur-sm rounded-lg border border-violet-800/30 p-6">
        <div className="flex flex-col md:flex-row gap-4">
          ...
          />
          <Button onClick={handleCreateWebhook} className="bg-violet-600 hover:bg-violet-700 text-white">
        ...
        {webhooks.length > 0 && (
          <div className="mt-6">
            <h4 className="text-lg font-medium text-white mb-3">Created Webhooks:</h4>
